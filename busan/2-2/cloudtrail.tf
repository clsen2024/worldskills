...
data "aws_iam_policy_document" "cloudtrail_policy" {
  statement {
    ...
    effect    = "Allow"
    resources = ["${aws_cloudwatch_log_group.cloudtrail.arn}:log-stream:${local.account_id}_CloudTrail_ap-northeast-2*"]
...
data "aws_iam_policy_document" "main" {
  statement {
    ...
    condition {
      ...
      values   = ["arn:aws:cloudtrail:ap-northeast-2:${local.account_id}:trail/wsi-project-trail"]
    }
  }
