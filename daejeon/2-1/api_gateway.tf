...
data "template_file" "api" {
  ...
  vars = {
    ...
    delete_role_arn = aws_iam_role.dynamodb_delete.arn
...
resource "aws_iam_role" "dynamodb_delete" {
  name               = "DynamodbDeleteRole"
  assume_role_policy = data.aws_iam_policy_document.dynamodb_assume.json
}

data "aws_iam_policy_document" "dynamodb_delete" {
  statement {
    effect    = "Allow"
    resources = [aws_dynamodb_table.main.arn]
...
resource "aws_iam_role_policy" "dynamodb_delete" {
  name   = "DynamodbDeletePolicy"
  role   = aws_iam_role.dynamodb_delete.id
  policy = data.aws_iam_policy_document.dynamodb_delete.json
}
