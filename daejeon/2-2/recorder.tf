...
resource "aws_cloudwatch_log_group" "deny" {
  name = "/ec2/deny/port"
}

resource "aws_cloudwatch_log_stream" "deny" {
  name           = "deny-${aws_instance.test.id}"
  log_group_name = aws_cloudwatch_log_group.deny.name
}